...
class GCloudLogger {
    ...
    private format(severity: Severity, message: unknown, ...args: unknown[]): string {
        const msg = `[${this.category}] ${message} ${args.filter((arg) => !this.isObject(arg)).join(' ')}`.trimEnd();
            ...
            (acc, arg) => ({
                ...
                ...Object.entries(arg).reduce(
                    (acc, [k, v]) => ({
                        ...
                        [k]:
                            v instanceof Error
                                ? { name: v.name, message: v.message, details: { ...v }, stack: v.stack }
                                : v,
                    }),
                    {}
                ),
    ...
    error(message: unknown, ...args: unknown[]): void {
        const _args =
            message instanceof Error ? [...args, { error: message, callstack: new Stacktrace().stack }] : [...args];
...
export const appLogger = isProduction ? new GCloudLogger('app') : getLogger('application');
export const accessLogger = isProduction ? new GCloudLogger('access') : getLogger('access');
